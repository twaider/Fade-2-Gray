...
static void window_load(Window *window) {
  ...
  s_weathertext_layer = text_layer_create(GRect(window_bounds.size.w / 2 + 5,
                                                PBL_IF_ROUND_ELSE(63, 63),
  ...
  s_time_font = fonts_load_custom_font(
      resource_get_handle(RESOURCE_ID_FONT_PIXEL_MIL_52)); // FONT_PIXEL_MIL_24
  ...
  layer_add_child(window_get_root_layer(window),
                  text_layer_get_layer(s_weather_icon_layer));
}
